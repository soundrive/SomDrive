...
export default function ArtistPublic({
  ...
  if (!artist) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4">
        ...
        <h3 className="text-xl font-heading font-bold text-center">Buscando pen drive musical...</h3>
      </div>
    );
  }
  ...
  const handleShareWhatsApp = () => {
    ...
    const text = encodeURIComponent(`🎙️ Ouça agora mesmo as músicas exclusivas do artista ${artist.name} diretamente no Bluetooth do seu carro ou aparelho! Acesse: ${pageUrl}`);
      ...
      <div id="artist-profile-header-card" className="max-w-4xl mx-auto px-6 pt-10">
        ...
        <div className="bg-slate-900 border border-slate-850 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 md:gap-8 shadow-2xl relative">
          ...
          <div className="text-center md:text-left space-y-3 flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-1.5 shrink-0">
              <button 
                id="pub-talk-whatsapp-btn"
                onClick={handleSpeakWithArtist}
                className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white rounded-lg text-xs font-heading font-black tracking-wider uppercase shadow-lg shadow-emerald-500/10 flex items-center gap-1.5 cursor-pointer select-none transition-transform active:scale-95"
              >
              ...
              <button 
                ...
                className="p-2 border border-slate-800 hover:border-slate-700 hover:bg-slate-850 rounded-lg text-slate-400 hover:text-white transition cursor-pointer"
              ...
              {artist.instagram && (
                <button 
                  ...
                  className="p-2 border border-slate-800 hover:border-slate-700 hover:bg-slate-850 rounded-lg text-slate-400 hover:text-white transition cursor-pointer flex items-center gap-1.5 text-xs font-mono"
